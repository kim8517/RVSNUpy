...
for i, age in enumerate([3, 5, 7, 9, 11]):
    wavelength, flux = np.loadtxt(os.path.join(os.path.dirname(__file__), 'fsps_absspec.txt'), usecols=[0,i+1], unpack=True)
    error, mask = copy.deepcopy(flux)/5, np.ones_like(flux)
    temp = np.vstack([wavelength, flux, error, mask])
    ...
    syn_abstemplates[f'{age:d}Gyr'] = [new_temp, 0.7, 1]

syn_emtemplates = {}
for i, age in enumerate([0.01]):
    wavelength, flux = np.loadtxt(os.path.join(os.path.dirname(__file__), 'fsps_emspec.txt'), usecols=[0,i+1], unpack=True)
    error, mask = copy.deepcopy(flux)/5, np.ones_like(flux)
    temp = np.vstack([wavelength, flux, error, mask])
    new_wavelengths = np.logspace(np.log10(temp[0,0]), np.log10(temp[0,-1]), len(temp[0]))
    new_fluxes, new_error = resampler(temp[0], temp[1], new_wavelengths), resampler(temp[0], temp[2], new_wavelengths)
    new_temp = np.vstack([new_wavelengths, new_fluxes, new_error, np.ones_like(new_wavelengths)])
    syn_emtemplates[f'{age:.2f}Gyr'] = [new_temp, 0.7, 2]
...
#     template = resampler(_template, linear_wavelength)
    ...
    pkfrac = 0.6
    sdss_galaxy_templates[template_name] = [template, pkfrac, 1]
...
#     template = resampler(_template, linear_wavelength)
    ...
    pkfrac = 0.6
    sdss_galaxy_templates[template_name] = [template, pkfrac, 2]
...
def zp_calib(templates):
    ...
    for name in abs_templates.keys():
        ...
        calib = abssyn.z_single(temp0, chi_thres=0)
        ...
        new_temp = np.vstack([new_wavelengths, new_fluxes, new_error, np.ones_like(new_wavelengths)])
        zp_templates[name] = [new_temp, sdss_galaxy_templates[name][1], sdss_galaxy_templates[name][2]]
        
    for name in em_templates.keys():
        zp_templates[name] = [templates[name][0], templates[name][1], templates[name][2]]
    return zp_templates
...
#     pkfrac = 0.7
#     sdss_template_QSO[template_name] = [template, pkfrac, 2]
